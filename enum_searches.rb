...
class Metasploit3 < Msf::Post
  ...
  def find_users(table)
    ...
    subkeys = registry_enumkeys(reg_path)
    subkeys.each do |user_sid|
      if user_sid.length > 10 and not user_sid.nil?
        ...
        find_searches(user_sid, username, table)
      end
    end
  end

  def find_searches(user_sid, username, table)
